...
CapacitiveSoilMoistureSensor::CapacitiveSoilMoistureSensor(int pin,
                                                           int minSensorValue,
                                                           int maxSensorValue)
    : pin(pin), minSensorValue(minSensorValue), maxSensorValue(maxSensorValue) {
  // assert min < max
}
...
float CapacitiveSoilMoistureSensor::getMoisturePercentage() {
  ...
  float percentage =
      calibratedSensorValue / (maxSensorValue - minSensorValue) * 100;
